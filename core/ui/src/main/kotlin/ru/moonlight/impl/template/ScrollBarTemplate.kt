...
/* author - https://stackoverflow.com/questions/66341823/jetpack-compose-scrollbars */

/**
 * Renders a scrollbar for LazyStaggeredGridState.
 *
 ...
 *
 * @param state The [LazyStaggeredGridState] that has been passed into the lazy staggered grid
...
fun Modifier.scrollbar(
    ...
    knobColor: Color = MoonlightTheme.colors.hintText,
    ...
    return drawWithContent {
        ...
        state.layoutInfo.visibleItemsInfo.firstOrNull()?.let { firstVisibleItem ->
            if (state.isScrollInProgress || alpha > 0f) {
                ...
                // this scrollbar.
                val viewportSize = if (horizontal) size.width
                                   else { size.height } - padding.toPx() * 2
                ...
                val viewportOffsetInFullListSpace =
                    state.firstVisibleItemIndex * firstItemSize.height + state.firstVisibleItemScrollOffset

                // Where we should render the knob in our composable.
                val knobPosition =
                    (viewportSize / estimatedFullListSize.height) * viewportOffsetInFullListSpace + padding.toPx()
                // How large should the knob be.
                val knobSize = fixedKnobRatio?.let { it * viewportSize }
                    ?: maxOf(32.dp.toPx(), (viewportSize * viewportSize) / estimatedFullListSize.height)
                ...
                drawRoundRect(
                    ...
                    size = if (horizontal) Size(size.width - padding.toPx() * 2, thickness.toPx())
                ...
                drawRoundRect(
                    ...
                    size = if (horizontal) Size(knobSize, thickness.toPx())
