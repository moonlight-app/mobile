...
internal suspend fun <T> safeApiCall(call: suspend () -> Response<T>): ApiResponse<T> {
    return try {
        val response = call()
...
internal suspend fun <T> safeApiCallWithHeader(call: suspend () -> Response<T>): ApiCall<T> {
    return try {
        val response = call()
...
internal suspend fun safeApiCallWithoutBody(call: suspend () -> Response<ResponseBody>): ApiResponse<Nothing> {
    return try {
        val response = call()
        if (response.isSuccessful) {
            ApiResponse.Success()
